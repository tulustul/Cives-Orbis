...
export class InterceptTask extends AiTask<
  InterceptTaskOptions,
  InterceptTaskSerialized
> {
  ...
  private predictTargetPath() {
    ...
    if (enemyCities.length === 0) {
      ...
      let closestDistance = this.lastKnownPosition.getDistanceTo(
        closestCity.tile,
      );
      ...
      this.predictedPath =
        path && path.length > 0 ? path[0] : [this.lastKnownPosition];
    }
    ...
    if (this.predictedPath.length > 0) {
      ...
      for (const point of this.predictedPath) {
        ...
        const targetDistance = this.lastKnownPosition
          ? this.lastKnownPosition.getDistanceTo(point)
          : 0;
  ...
  serialize(): InterceptTaskSerialized {
    return {
      ...
      interceptors: this.interceptors
        .map(unitToIdAndName)
        .filter((u): u is UnitIdAndName => u !== null),
