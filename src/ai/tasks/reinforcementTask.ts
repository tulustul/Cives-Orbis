...
export class ReinforcementTask extends AiTask<ReinforcementTaskOptions, ReinforcementTaskSerialized> {
  ...
  private selectReinforcements() {
    ...
    const availableUnits: { unit: UnitCore; distance: number; score: number }[] = [];
  ...
  serialize(): ReinforcementTaskSerialized {
    return {
      ...
      state: this.state,
      reinforcements: this.reinforcements.map(unitToIdAndName),
