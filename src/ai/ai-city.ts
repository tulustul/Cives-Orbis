...
export class CityAI extends AISystem {
  ...
  private buildingCategoryKeywords = {
    [BuildingCategory.FOOD]: ['farm', 'granary', 'food', 'growth', 'irrigation'],
    [BuildingCategory.PRODUCTION]: ['workshop', 'factory', 'production', 'forge', 'industry'],
    [BuildingCategory.SCIENCE]: ['library', 'university', 'academy', 'science', 'knowledge', 'research'],
    [BuildingCategory.DEFENSE]: ['wall', 'fortress', 'barracks', 'defense', 'military'],
    [BuildingCategory.ECONOMY]: ['market', 'trade', 'economy', 'gold', 'wealth', 'bank', 'commerce'],
  ...
  private updateCitySpecializations() {
    // Get strategic focus if available
    let strategicFocus = "balanced";
    const strategicAI = this.ai.systems.find(sys => sys instanceof StrategicAI) as StrategicAI | undefined;
    if (strategicAI) {
      strategicFocus = strategicAI.getStrategicFocus();
    }
    ...
    for (const city of this.player.cities) {
      ...
      if (currentTurn - info.lastEvaluatedTurn >= 5) {
        ...
        info.specialization = this.determineCitySpecialization(city, strategicFocus);
        // Update buildings needed
        info.buildingsNeeded = this.evaluateCityBuildingNeeds(city, info.specialization);
   ...
   */
  private determineCitySpecialization(city: CityCore, strategicFocus: string): CitySpecialization {
    ...
    const avgPopulation = this.player.cities.reduce((sum, c) => sum + c.population.total, 0) / 
  ...
  private evaluateCityBuildingNeeds(
    ...
    if (city.population.total < 3) {
      needs.set(BuildingCategory.FOOD, Math.max(needs.get(BuildingCategory.FOOD)!, 25));
    }
    
    // If city has lots of resources, boost economy
    const resourceCount = Array.from(city.expansion.tiles).filter(t => t.resource).length;
    if (resourceCount >= 3) {
      needs.set(BuildingCategory.ECONOMY, Math.max(needs.get(BuildingCategory.ECONOMY)!, 20));
    }
    
    // If this is the capital, boost science
    if (city === this.player.cities[0]) {
      needs.set(BuildingCategory.SCIENCE, Math.max(needs.get(BuildingCategory.SCIENCE)!, 20));
      needs.set(BuildingCategory.UTILITY, Math.max(needs.get(BuildingCategory.UTILITY)!, 15));
    }
  ...
  private categorizeBuilding(building: any): BuildingCategory {
    ...
    for (const [category, keywords] of Object.entries(this.buildingCategoryKeywords)) {
  ...
  getCitySpecialization(cityId: number): string {
    return this.cityInfo.get(cityId)?.specialization || CitySpecialization.GENERAL;
  }
}